...
public class StructureImporterTest {
	...
	public void testStructureXMLParsing() throws Exception {
		InputStream xmlIN = new FileInputStream("/home/renato/development/forpdi/forpdi-repo/backend-java/src/main/resources/readedStructure-unifal.xml");
		...
		out.printf("{\nStructure:\n\tname:%s\n\tdescription:%s\n\tlevels:[\n", importer.readedStructure.name, importer.readedStructure.description);
		for (StructureBeans.StructureLevel level : importer.readedStructure.levels) {
			...
			for (StructureBeans.Attribute attr : level.attributes) {
				out.printf("\t\t{\n\t\t\tlabel:%s\n\t\t\tdescription:%s\n\t\t\ttype:%s\n",
					attr.label, attr.description, attr.type);
				out.printf("\t\t\trequired:%s\n\t\t\tvisibleInTables:%s\n\t\t}\n",
					attr.required ? "true":"false",attr.visibleInTables ? "true":"false");
			}
			out.printf("\t\t]\n\t}\n");
		}
		out.printf("\t]\n}\n");
