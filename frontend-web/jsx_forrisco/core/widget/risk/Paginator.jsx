...
export default React.createClass({
	...
	componentDidMount(){
		this.setState({
			pages: Math.ceil(this.props.totalOfRisks/this.state.pageSize)
		});
	},

	componentWillReceiveProps(newProps){
		this.setState({
			pages: Math.ceil(newProps.totalOfRisks/this.state.pageSize),
			page: (this.props.page !== undefined ? this.props.page : (this.state.page > Math.ceil(newProps.totalOfRisks/this.state.pageSize) ? 1 : this.state.page))
		});
	},

	loadPage(page, size){
		size = (isNaN(size) ? this.state.pageSize : size);
		page = (page > Math.ceil(this.props.totalOfRisks/size) ? Math.ceil(this.props.totalOfRisks/size) : page);
		this.props.onChangePage(page, size);
		this.state.page = page;
	},

	renderPages(){
		var max = Math.ceil(this.props.totalOfRisks/this.state.pageSize);
		var pages = [];
		if(max <= 10){
			for (var i = 1; i <= max; i++) {
				pages.push(
